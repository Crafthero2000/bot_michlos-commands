local command = { "кф", "Коинфлип", use = "<профиль> [ставка]", smile = '&#127767;', _type = 'games' };
...
function command.exe(msg, args, other, rmsg, user)
	...
	if user.balance < count then return "err:у вас нет столько бит" end
	if count > 2000 then return "err:вы не можете ставить так много бит" end
	if count < 5 then return "err:вы не можете ставить так мало бит" end
	...
	rmsg:line("&#128176; Ставка: " .. count .. " бит.");
	rmsg:line("&#9899; %s должен принять запрос командой:", target:r());
	rmsg:line("&#10145; принять " .. qid);
end

command.accept = function (count, target, source, rmsg)
	if target.balance < count then return "err:у вас нет столько бит" end
	if source.balance < count then return "err:оппонент успел потратить свои биты" end
